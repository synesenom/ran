...
- `test/precision-continuous.js`: a precision regression gate pinning pdf, cdf, and quantile accuracy for all 110 continuous distributions against external high-precision reference values (mpmath 1.4.1, `mp.dps = 50`). Each distribution is checked at 3 parameter sets × 5 interior x-values (the `F⁻¹(p)` for `p ∈ {0.1, 0.3, 0.53, 0.72, 0.9}` — off the exact centre so symmetric medians and degenerate central points are avoided — so every probe lands strictly inside the support); parameter-free distributions (Gilbrat, HalfLogistic, …) naturally use a single set. pdf/cdf assert relative error ≤ 1e-14 (a documented 1e-12 cap applies to series/cancellation-limited families), and quantile is verified by the round-trip `q(cdf(x)) = x` at a per-distribution empirical tolerance (1e-14 for closed-form/Halley quantiles, a documented looser bound where ranjs computes the quantile by root-finding or approximation). Reference values are generated by `scripts/precision-refs-continuous.py`, which self-checks every formula against the external parameterization in `test/dist-cases-continuous.js`. Companion to the discrete gate in `test/precision-discrete.js` and the special-function gate in `test/precision.js` (#633).
- `test/precision-discrete.js`: a precision regression gate pinning pmf, cdf, and quantile accuracy for all 28 discrete distributions against external high-precision reference values (mpmath 1.4.1, `mp.dps = 50`). Each distribution is checked at 3 parameter sets × up to 5 k-values spanning the support; pmf/cdf assert relative error ≤ 1e-14 (a documented 1e-12 cap applies to ten log-gamma/Bessel-based families that cannot reach 1e-14), and quantile is probed at the midpoint of each probability step. Reference values are generated by `scripts/precision-refs-discrete.py`, which self-checks every formula against the external parameterization in `test/dist-cases-discrete.js`. Companion to the special-function gate in `test/precision.js` (#634).

### Fixed

- `betaIncomplete(a, b, x)` backward branch (triggered when `x >= (a+1)/(a+b+2)` and `b != 0`) now returns `B(a,b) − bt·CF/b` instead of `1 − bt·CF/b`, correctly applying the complement identity `B(a,b,x) = B(a,b) − B(b,a,1−x)` for the unnormalized function. The previous code substituted `1` for `B(a,b)`, which is valid only for the regularized variant. Example: `betaIncomplete(2, 3, 0.5)` now returns `11/192 ≈ 0.0573` instead of `≈ 0.974` (#675).
