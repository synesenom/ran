...
3. **Do you have the gradient of the log-density?** → `NUTS` is the default (self-tuning trajectory length via the doubling-tree/U-turn criterion); `HMC` if you want the trajectory length under direct control; `MALA` if a single Langevin step per iteration is enough. **Note:** neither `HMC` nor `NUTS` adapts a Euclidean metric yet — momentum is drawn from a standard Normal (identity mass matrix), so parameters spanning very different scales will mix unevenly (tracked in [#826](https://github.com/synesenom/ran/issues/826)); reparameterize or rescale until then.
