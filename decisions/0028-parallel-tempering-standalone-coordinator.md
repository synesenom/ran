...
Issue #830 asks for a Parallel Tempering (Replica Exchange MCMC, Geyer 1991) sampler: N replicas of a chain run at inverse temperatures β₁=1 > β₂ > … > βₙ, each targeting `βᵢ · log p(x)`. Periodically, adjacent replicas' current positions are proposed for a swap, and the issue states the acceptance probability as `min(1, exp((βᵢ − βᵢ₊₁)(log p(xᵢ) − log p(xᵢ₊₁))))`, using the *unscaled* `log p`. The cold (β=1) replica's samples are the output.

**The issue's formula as quoted above is mathematically incorrect** — `xᵢ` and `xᵢ₊₁` are swapped relative to the ratio that detailed balance on the joint replica distribution actually requires (derived below in "Decision"). Implementing the issue's literal formula was verified, empirically, to produce a wrong stationary distribution: the cold replica's within-mode variance came out inflated (~1.38 vs. the true 1.0) against a bimodal-target KS test built during implementation. This ADR documents and the shipped code implements the corrected formula, not the issue's literal text; see the comment on `_proposeSwap` in `src/mc/parallel-tempering.js` and the note left on issue #830 itself.
...
**The acceptance formula, corrected, is `min(1, exp((βᵢ − βᵢ₊₁)(log p(xᵢ₊₁) − log p(xᵢ))))`** — note `log p(xᵢ₊₁)` first, the reverse of the issue's text. Deriving from detailed balance on the joint distribution `Π(x₁,...,xₙ) = ∏ₖ p(xₖ)^{βₖ}`: swapping replica i's position `a` with replica i+1's position `b` changes `Π` from `p(a)^{βᵢ} · p(b)^{βᵢ₊₁}` to `p(b)^{βᵢ} · p(a)^{βᵢ₊₁}`, giving an acceptance ratio of `exp[(βᵢ − βᵢ₊₁)(log p(b) − log p(a))] = exp[(βᵢ − βᵢ₊₁)(log p(xᵢ₊₁) − log p(xᵢ))]`.
