...
# The coefficient table is inlined into marcum-q.js between these markers --
# every other special function inlines its coefficient table in its own file,
# so there is no standalone module. emit() splices the block in place.
BEGIN_MARKER = ('// --- BEGIN GENERATED COEFFICIENTS: '
                'scripts/marcum-fjk-generate.py ---')
END_MARKER = '// --- END GENERATED COEFFICIENTS ---'
...
def emit(fjk, jmax, kmax):
    ...
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        '..', 'src', 'special', 'marcum-q.js')
    with open(path) as fh:
        src = fh.read()
    i = src.index(BEGIN_MARKER)
    j = src.index(END_MARKER) + len(END_MARKER)
    with open(path, 'w') as fh:
        fh.write(src[:i] + block + src[j:])
    print('  spliced coefficients into src/special/marcum-q.js  '
