...
- `Lindley`, `Moyal`, and `Reciprocal` now implement exact closed-form quantile functions (`_q`) instead of falling back to the Chandrupatla root-finder: `Lindley` inverts via the `W₋₁` branch of Lambert W, `Moyal` via `erfinv`, and `Reciprocal` (log-uniform) directly as `a·(b/a)^p`. Each inversion mirrors the distribution's existing inverse-transform sampler, so `q()` is now both faster and accurate to machine precision.
