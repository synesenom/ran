...
# groups (tol: 1e-3, qtol: 1e-4) in test/precision-continuous.js because the group-level
# tolerance would otherwise downgrade the 5 interior probes from 1e-14 to 1e-3.
...
_N_NORMAL_TAIL = 'far-tail probes (p≈1e-7 and 1e-12): CDF = 0.5*(1+erf(z)) cancels ~12 digits for z<-3.5; peak error ~1.24e-4 at -7sigma. q() Newton steps hit the same erf path; peak round-trip error ~4e-5'
NOTES = {
