...
| Item | File | What to add |
| --- | --- | --- |
| Reference values | `test/process-cases.js` | Entry with `name`, `ctor`, and `refs`. Each ref carries `should` (prose completing the mocha title), `params` (a **thunk** returning constructor arguments, so no instance — and no PRNG — is built at import), `method`, `args`, optional `chain`/`chainArgs` (for `marginal(t).pdf(x)`), `expected`, `tol`, and `source`. `source` is passed as the assertion message, so provenance shows up in failure output instead of only in a comment a reader has to find. Consumed by a plain inline `forEach` in `test/process/reference-values.js` — there is no `dist-runner.js`-style shared runner, and at 9 processes there should not be one. |
| Precision gate | `test/precision-process.js` | Generated — see below. |
| Behavioural tests | `test/process/<name>.js` | Contract behaviour the precision gate does not probe: `NaN` for `t <= 0`, exact `0` outside the support, `Infinity` at a collapsed endpoint, symmetry about the mean. Sampled moments go through `assertSampleMoments` at the fixed `MOMENT_SEEDS = [0, 42, 12345]`, compared against a closed form derived independently from the SDE with a CLT-derived `K_SIGMA = 8` tolerance — never against the process's own `mean()`/`variance()`, which would be a tautology. Both helpers, along with the other shared test utilities in that describe block, live in `test/process/_helpers.js`. |
...
- `test/process/` — one file per process (`brownian-motion.js`, `ar1.js`, etc.), a `process.js` for the abstract `Process` base class, a `_helpers.js` for shared sampling/assertion helpers (`assertSampleMoments`, `MOMENT_SEEDS`, `K_SIGMA`, etc.), and `reference-values.js` consuming `test/process-cases.js` — mirrors the `test/mc/` per-sampler layout.
- `test/process-cases.js` — externally-sourced reference values for `ran.process` analytical methods (issue #1221), consumed by a plain inline `forEach` in `test/process/reference-values.js`.
...
### Verifying refactors of shared test infrastructure

A refactor of a file that test code depends on (`test/test-utils.js`, `test/dist-runner.js`, `test/process/_helpers.js`, `test/mc/_helpers.js`, or any other shared assertion helper) carries a failure mode ordinary `npm test` cannot see: the suite can stay 100% green while an assertion has quietly become powerless (a dropped `assert`, a `continue`→`return` conversion that returns before the check, a comparison degenerated into a tautology). A passing run only proves the current code satisfies whatever the tests check — it does not prove the tests can still fail. Before reporting such a refactor complete, run both:

- **Golden-master diff (proves the refactor preserved behavior).** Load the pre-refactor version of the file (`git show <parent-commit>:<path>`) alongside the refactored version, run the same exported entry points against a handful of real, representative inputs chosen to hit every branch (e.g. one smooth case, one with a kink/edge condition, one at a saturation/boundary limit), and diff the **exact sequence of calls** each version makes into its collaborators (not just the pass/fail verdict) — wrap the collaborator (e.g. `dist.pdf`/`dist.cdf`) in a spy that logs every call and argument, and assert the logs are identical. A verdict-only diff can still hide a divergence that happens not to flip any assertion on the chosen inputs; a call-log diff catches control-flow drift (off-by-ones, skipped branches, reordered iteration) directly.
- **Mutation test (proves the tests still have teeth).** Temporarily inject a real, wrong-but-plausible bug into the production code the helper is meant to catch (e.g. scale a `_pdf` by 1.05), rerun the affected slice of the suite, confirm it fails through the refactored helper specifically (check the stack trace names the helper), then revert the injected bug and confirm a clean `git diff`. Do this once per independent code path the refactor touched — a single mutation on one branch does not vouch for a sibling branch (e.g. mutating a continuous distribution's pdf says nothing about whether the discrete branch of the same helper still asserts correctly).

Neither technique substitutes for the other: golden-master diffing proves equivalence to the old implementation; mutation testing proves the new implementation still detects real defects. Both are throwaway — write the harness in the scratchpad or as a temporary file removed before commit, never as a committed test (it asserts against the file's own git history, not a fixed specification, so it has no ongoing value once the refactor is verified).
