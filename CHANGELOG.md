...
- `JohnsonSU._fitInit` and `JohnsonSB._fitInit` added using the Slifker-Shapiro (1980) quantile method: four symmetric sample quantiles at Φ(±z)/Φ(±3z) levels (z=0.5) back-compute γ, δ, λ, ξ in closed form for SU; for SB, OLS on logit-transformed quantiles gives γ and δ with bounds seeded from the sample range (#440).
- `_fitInit` data-aware method-of-moments initializers added to 9 noncentral distributions (`NoncentralChi2`, `NoncentralChi`, `NoncentralT`, `NoncentralBeta`, `NoncentralF`, `DoublyNoncentralChi2`, `DoublyNoncentralBeta`, `DoublyNoncentralF`, `DoublyNoncentralT`); degrees-of-freedom parameters are seeded from central-distribution moment equations and noncentrality parameters from the deviation of sample statistics from those central-case estimates (#439).
...
- Catastrophic cancellation in `FlorySchulz._cdf` for small `a` fixed. The naive formula `1 − (1−a)^k·(1+ka)` subtracted nearly-equal terms, losing all significant digits when `a → 0`. Rewritten as `-expm1(k·log1p(−a)) − exp(k·log1p(−a))·ka`, both terms well-conditioned for small `a` and small `k`. Boundary-region reference values at `a=1e-8` for k=1, 2, 5 added to the test suite. Closes #248.
- Catastrophic cancellation in `Moyal._cdf` at very negative `x` fixed by computing the upper regularized incomplete gamma Q(½, z) via `gammaUpperIncomplete` directly instead of `1 - gammaLowerIncomplete(...)`. When `z = ½·exp((μ−x)/σ)` is large (x ≪ μ), the old formula computed `1 - (1 - Q)`, losing all precision; the fix routes through the continued-fraction path and returns the correct sub-1e-9 value at full double precision. Boundary-region reference values at `x=−10` (CDF ≈ 3.85e-34), `x=−6` (CDF ≈ 7.41e-6), and `x=−4` (CDF ≈ 6.56e-3) added to the test suite for params (μ=0, σ=2). Closes #247.
...
- Catastrophic cancellation in `NoncentralChi2._cdf` at small `x` fixed by computing the complementary Marcum Q (`marcumP`) directly instead of `1 - marcumQ(...)`. Previously, `NoncentralChi2(11, 2).cdf(0.01)` returned `0` (or `1.11e-16`) due to a double subtraction-from-1 chain — `marcumQ` internally forms `1 - P` to deliver `Q`, then `_cdf` formed `1 - Q` again — destroying precision in the lower tail. It now returns the correct value `≈ 2.81e-16` at full double precision. A new `marcumP` function is exported from `src/special/marcum-q.js` for any caller that needs `1 − Q_M(a, b)` without the cancellation. Boundary-region reference values at `x=1e-4` (CDF ≈ 2.82e-27) and `x=1e-2` (CDF ≈ 2.81e-16) added to the test suite. Closes #245.
- Catastrophic cancellation in `InverseGamma._cdf` at small `x` fixed by computing the upper tail directly via `gammaUpperIncomplete(alpha, beta/x)` instead of `1 - gammaLowerIncomplete(...)`. When `beta/x` is large (x near 0), the old formula returned `1 - 1 = 0` with complete loss of precision; it now returns the correct value via the numerically stable continued-fraction path. Boundary-region reference values at `x=0.05` and `x=0.1` added to the test suite. Closes #244.
...
- scipy/numpy reference values (`refVals`) added to 8 core discrete distributions: `Bernoulli`, `Binomial`, `Categorical`, `DiscreteUniform`, `Geometric`, `Hypergeometric`, `NegativeBinomial`, `Poisson`. Cross-validated against `dist/ranjs.cjs.js` within `1e-10`. Three non-trivial scipy parameter mappings are documented in `solutions/testing/2026-05-18-1443-discrete-refvals-scipy-parameterization-traps.md`: Geometric's 0- vs 1-indexed convention (use `nbinom(1, p)`), NegativeBinomial's success/failure swap (`nbinom(r, 1-p)`), and Hypergeometric's `n`/`N` name collision with ranjs. Closes #134.
