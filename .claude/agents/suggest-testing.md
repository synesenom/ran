...
description: Scans test suite for coverage gaps, missing statistical correctness scenarios, and test infrastructure improvements, across distributions, processes, MCMC samplers, and summary/dependence statistics and hypothesis tests.
model: sonnet
tools:
  - Read
  - Glob
...
Scan the test suite and production code to find areas where testing could be strengthened — not just structural coverage but statistical and numerical correctness scenarios. `location`, `dispersion`, `shape`, `dependence`, and `test` (`src/test/`'s hypothesis tests) are first-class targets here, held to the identical rigor bar as `src/dist/`, `src/process/`, and `src/mc/`: externally-sourced (mpmath/scipy/R) reference values at a real precision gate, not a handful of uncited spot-checks. Do not treat these five as a lower tier just because they lack an abstract base class — a missing precision gate or a missing Type-I-error check there is exactly as serious a gap as a missing `ksTest` recovery check would be for a distribution.
...
- `src/location/`, `test/location.js` — Central-tendency measures and their tests
- `src/dispersion/`, `test/dispersion.js` — Spread/inequality measures and their tests
- `src/shape/`, `test/shape.js` — Shape/order-statistic measures and their tests
- `src/dependence/`, `test/dependence.js` — Association/dependence measures and their tests
- `src/test/`, `test/test.js` — Hypothesis tests (bartlett, levene, brownForsythe, mannWhitney, hsic, welch, andersonDarling, cramerVonMises, kolmogorovSmirnov) and their tests
- `test/precision-continuous.js`, `test/precision-discrete.js`, `test/precision-special.js`, `scripts/precision-refs-continuous.py`/`-discrete.py`/`-special.py` — the established precision-gate pattern (mpmath `mp.dps=50`, generated via a Python reference script) that `location`/`dispersion`/`shape`/`dependence` currently lack entirely
...
1. **Scan the test structure**:
   ...
   - Check `test/location.js`, `test/dispersion.js`, `test/shape.js`, `test/dependence.js`, and `test/test.js` for the same — specifically, whether each numeric literal in a `closeTo`/equality assertion carries a comment naming its external source (mpmath/scipy/R), per CLAUDE.md's Testing Conventions
...
2. **Read key test files** to understand:
   ...
   - For `location`/`dispersion`/`shape`/`dependence`: whether a real precision gate exists (multiple input sizes/shapes, externally-cited values) or the file only has a handful of small hand-computed arrays
   - For `test/test.js`'s hypothesis tests: whether each test has both a deterministic reference statistic/p-value (external-tool-cited) AND a statistical size/power check (empirical rejection rate across many seeds under the null), or only a single seeded pass/reject assertion
...
4. **Read corresponding production code** to identify:
   ...
   - `location`/`dispersion`/`shape`/`dependence` functions whose only tests are uncited numeric literals or trivial hand-computable arrays, where an mpmath/scipy/R precision gate (matching the distribution standard) is achievable and missing
   - `src/test/` hypothesis tests whose "should pass/reject" coverage is a single seeded sample rather than an empirical Type-I-error/power simulation
...
5. **Identify gaps and opportunities**, drawing on both the code scan and `todo.md`:
   ...
   - `location`/`dispersion`/`shape`/`dependence` functions lacking a precision gate against mpmath/scipy/R at varied input sizes (not just n≤8 hand arithmetic)
   - Uncited numeric literals in `test/dispersion.js`/`test/dependence.js` (or elsewhere) that violate the "externally-sourced reference value" rule
   - `src/test/` hypothesis tests lacking a Type-I-error/power simulation (empirical rejection rate across many seeds vs. nominal alpha) — `cramerVonMises`/`kolmogorovSmirnov`'s scipy-cited assertions in `test/test.js` are the rigor bar the rest of the suite should reach, not the ceiling
...
- Do not treat `location`/`dispersion`/`shape`/`dependence`/`test` as out of scope or secondary either — they currently lack the precision-gate/externally-cited-reference-value rigor distributions, processes, and MCMC samplers already have, which makes them an especially high-value target right now, not a lower one
- Do not suggest a family for the sake of balance — if the strongest gaps this run are concentrated in one or two of `location`/`dispersion`/`shape`/`dependence`/`test`, say so; don't pad with a weak suggestion elsewhere just to cover every family
