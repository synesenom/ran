...
| [`/release`](skills/release/SKILL.md) | preflight → version bump → changelog → PR → merge (all via GitHub MCP) → trigger `release.yml` (tag, npm publish, GitHub release, milestone rotation, release-branch cleanup) | Cut a versioned npm release end-to-end |
...
/release ────→ (no agents; GitHub MCP for PR/merge, then triggers .github/workflows/release.yml for tag/publish/release/milestone/branch-cleanup)
