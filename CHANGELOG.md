...
- `fit()` on Beta-family distributions (`Beta`, `BetaRectangular`, `BetaPrime`, and other `Beta` subclasses) no longer converges to near-singular shape parameters. A Jeffreys-like log-barrier penalty `−0.5·(log α + log β)` is added to the optimisation objective via a new `static _fitPenalty(dist)` hook on the `Distribution` base class, overridden in `Beta`. The penalty is additive (zero at `α = β = 1`, diverging to `+∞` as `α → 0⁺` or `β → 0⁺`), so it repels the Powell optimizer from near-zero shapes while leaving well-interior solutions unchanged. The base-class default returns `0` (pure MLE), so no other distribution is affected. See ADR-0017 (#625). Five re-parametrizing Beta subclasses (`F`, `R`, `PERT`, `BaldingNichols`, `FisherZ`) now override `_fitPenalty` to return `0`, blocking the inherited log-barrier whose MAP bias was unintended since their native `fit()` parameter spaces do not map one-to-one to `(alpha, beta)` (#660).
- `BetaBinomial`, `Hypergeometric`, and `NegativeHypergeometric` `_cdf(x)` now use bidirectional raw-PMF summation (`Math.min(1, Math.max(fwd, 1 − bwd))`) instead of the inherited `Categorical` prefix-sum table, fixing 1-ULP quantile overshoot at round probability boundaries caused by AliasTable normalisation bias. Fixes `BetaBinomial(25, 2, 2).q(0.5)` returning `13` (correct: `12`) and `NegativeHypergeometric(10, 5, 3).q(0.5)` returning `3` (correct: `2`) (#658).
...
- `beta(m, n)` now returns exact IEEE 754 results for small positive integer arguments (`min(m, n) ≤ 30`) via a direct recurrence `B(1,n)=1/n`, `B(m,n)=B(m−1,n)·(m−1)/(m+n−1)`, instead of routing through three `logGamma` Lanczos calls which accumulated a sub-ULP round-trip error. This fixes `YuleSimon(3).q(0.75)` returning `2` instead of the correct `1`: `beta(1,4)` was returning `0.25000000000000017` instead of `0.25`, causing `_cdf(1)` to land at `0.7499999999999998` and the discrete quantile search to advance past the correct bucket (#653).
- `hurwitzZeta` Bernoulli correction series now uses the exact ratio recurrence T_k/T_{k-1} = (B_{2k}/B_{2k-2})·(s+2k-3)(s+2k-2)/((2k)(2k-1))/N² instead of a running product of log-Gamma factors that silently collapsed to only the first correction term T₁. Combined with raising n_min from 20 to 50, this brings relative precision for s ∈ (1, 3] from ~1e-8 to within 1e-14 (#678).
...
- `riemannZeta` now uses a three-term Laurent expansion (`1/(s−1) + γ₀ − γ₁·(s−1) + γ₂·(s−1)²/2`, DLMF 25.2.8) for `s ∈ (0.99, 1.1001)`, extending the previous two-term expansion (`s ∈ (0.99, 1.01)`). The upper threshold is widened to cover the catastrophic-cancellation zone on the `s > 1` side; the lower threshold stays at `−0.01` because `1−2^(1−s)` is well-conditioned for `s < 0.99`. Improves accuracy from ~3e-8 relative (Wynn-epsilon) to <1e-14 for `s ∈ (1.01, 1.1]`, and removes the `s ≥ 1.5` workaround from the identity test `riemannZeta(s) − hurwitzZeta(s, n+1) = H(s, n)` (#642).
...
- `recursiveSum` iteration cap raised from 100 (`MAX_ITER`) to 500 (`MAX_SERIES_ITER`). The old cap silently truncated series that need O(|z|) or O(λ) terms to converge — specifically the Kummer ₁F₁ Taylor series for |z| close to 50 and the Poisson-weighted noncentral distribution CDFs at large noncentrality parameters (#564).
...
- `besselInu(nu, x)` now returns values accurate to near-machine precision (~1e-15 relative error) for large arguments (x = 50, 100, 200) at fractional orders ν = 0.5, 1.5, 2.3. The truncation bug (loop exiting before the series peak at i ≈ x/2) was already resolved by the `MAX_SERIES_ITER = 500` change in #564; explicit test assertions now lock in that fix for the fractional-order path (#629).
