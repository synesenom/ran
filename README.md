...
The stochastic processes in `src/process/` have the same gate in `test/precision-process.js`, generated by `scripts/precision-refs-process.py`. All 7 processes with a closed-form time-`t` marginal — AR1, BrownianBridge, BrownianMotion, CoxIngersollRoss, GeometricBrownianMotion, OrnsteinUhlenbeck, and Poisson — are verified at **1e-14** across three parameter sets × three times × five interior points each, with no looser exceptions. Every reference gates three independent code paths: `pdf(x, t)`, `marginal(t).pdf(x)`, and `marginal(t).cdf(x)`. Each marginal law is re-derived from the process's SDE inside the generator, which self-checks those derivations against externally sourced values before emitting a single literal.
