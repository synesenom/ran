...
3. **Do you have the gradient of the log-density?** → `NUTS` is the default (self-tuning trajectory length via the doubling-tree/U-turn criterion, plus the same `config.metric` Euclidean mass-matrix adaptation as `HMC`); `HMC` if you want the trajectory length under direct control; `MALA` if a single Langevin step per iteration is enough. Both `NUTS` and `HMC` accept `config.metric` (`'diag'` by default, `'dense'` for correlated parameters), adapted during warm-up, so a poorly-scaled or correlated target mixes well under either.
4. **No gradient — low or high dimensional?** → `Slice` for low-d (no proposal to tune, interval width self-adapts); `RWM` or `AdaptiveMetropolis` for higher-d (`AdaptiveMetropolis` when parameters are correlated, since it learns the full proposal covariance instead of a diagonal one).
...
All 31 discrete distributions are verified against mpmath references at 50 decimal places. BetaBinomial and NegativeHypergeometric sit at the ~2e-14 float64 arithmetic floor. The following distributions cap at 1e-12 at certain parameter settings: Binomial, Hypergeometric, NegativeBinomial, Poisson. Skellam caps at 1e-12 for most parameter settings, loosening to 1e-11 at highly asymmetric, large means (e.g. mu1=5000, mu2=1) where log-domain cancellation grows with mu1.
...
The 9 hypothesis tests in `src/test/` have an analogous precision gate in `test/precision-test.js`, generated by `scripts/precision-refs-test.py` and checked via `scripts/eval-test.js`. Unlike every other gate, references come from R 4.3.3 (base R + goftest 1.2.3) rather than mpmath, since these are canonical statistical procedures, not bare mathematical functions; `hsic` has no R equivalent and is instead an independent from-first-principles Python reimplementation of Gretton et al. (2008). Only `andersonDarling`, `cramerVonMises`, and `kolmogorovSmirnov` return a `pValue` field to check directly — the other 6 functions return only `{stat, passed}`, so `passed` is checked against an independently R-computed p-value/critical-value comparison. Documented convention gaps between ranjs and R's defaults (`mannWhitney` has no tie/continuity correction; `kolmogorovSmirnov` is always asymptotic; `cramerVonMises` omits R's finite-sample correction) are reconciled in the generator rather than papered over — see that script's module docstring for the full methodology and the two flagged (not fixed, per this issue's scope) numeric divergences: `andersonDarling`'s finite-`n` correction differs from R's own by up to ~7e-5 relative, and `ran.dist.Kolmogorov`'s survival function differs from R's Kolmogorov CDF by up to ~1e-4 relative at some arguments.
...
The tests above pin exact values at a curated set of points; [`docs/accuracy.md`](docs/accuracy.md) instead reports **measured** error bounds — in [ULP](https://en.wikipedia.org/wiki/Unit_in_the_last_place) against [mpmath](https://mpmath.org/) at `mp.dps = 50` — from a differential-testing harness that sweeps randomized inputs across a documented parameter and argument domain for each special function and distribution `pdf`/`pmf`/`cdf`.

A bound in that table holds only over the domain its row states as swept — it is not a claim about behavior outside that domain. A function or distribution the harness has not yet swept appears as **not yet measured** rather than being left out, and a known accuracy defect is reported with its actual bad value and a link to the tracking issue rather than rounded away. Treat an unmeasured entry as an open question, not a passing one.
...
The public API is considered stable: a breaking change first ships a deprecation warning and stays in place for at least one released minor version before removal, documented in `CHANGELOG.md` as it happens. Bug reports and security issues are triaged through GitHub issues. Releases are batched, not continuous — changes land on `main` under `[Unreleased]` and ship together in a dedicated release PR once enough have accumulated, not on every merge. There is no guaranteed response-time SLA.
