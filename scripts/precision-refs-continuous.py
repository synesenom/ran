...
# Kolmogorov regression point (issue #1412): pinned at the exact z = sqrt(n*e)*D value where R's
# own ks.test(..., exact=FALSE) asymptotic p-value was found to diverge from the theta series --
# see the _TOL_R_PKSTWO_ASYMPTOTIC_LIMIT comment in scripts/precision-refs-test.py for the full
# investigation. ranjs's src/dist/kolmogorov.js was NOT the imprecise side: an independent mpmath
# mp.dps=50 ground truth at this z confirmed ran.dist.Kolmogorov matches to float64 noise while R's
# pkstwo (tol=1e-6, six orders of magnitude looser than ranjs's Number.EPSILON-relative
# termination) was off by 9.65e-5. This single point is OUTSIDE P_GRID and is NEVER wired into
# compute_cache() (same reason as NORMAL_FAR_TAIL_XVALS/DNCT_NEGX_XVALS above -- it targets one
# specific z, not a parameter/probability sweep); the test/precision-continuous.js Kolmogorov
# group's extra point is hand-maintained and not regenerated by --emit. A companion dict is not
# required for a single point (contrast the *_XVALS dicts above, which each cover 3+ points across
# multiple parameter sets). Computed via mpmath at mp.dps=50 from the same theta series
# src/dist/kolmogorov.js implements:
#   pdf(x) = 8 * sum_{k=1}^inf (-1)^(k-1) * k^2 * x * exp(-2 k^2 x^2)
#   cdf(x) = 1 + 2 * sum_{k=1}^inf (-1)^k * exp(-2 k^2 x^2)
# Kolmogorov: x=0.98552065998180416 => pdf=1.1168412893932559  cdf=0.71415464855449845
